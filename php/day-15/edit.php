...
bodyStart(title: "Day 15 - Edit", useWater: false, styles: $css);

$studentModel = new DB("students");

$student = $studentModel->find($_REQUEST['id']);
...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
  ...
  } else $data["image"] = $student->image;

  if (!sizeof($errors)) {
    $result = $studentModel->update($student->id, $data);
    if ($result) {
      tag(
        "script",
        <<<JS
...
<main class="container">
  ...
  <section>
    <form action="<?= $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
      <input type="hidden" name="id" value="<?= (int) old('id', $student->id) ?>">
      <label for="name">
        Name
      ...
      <label for="department">
        ...
        <select id="department" name="department_id"
          ...
          <?php
          $depID = (int) old('department_id', $student->department_id);
          foreach ($departments as $dept): ?>
            <option value="<?= $dept->id ?>" <?= $depID === $dept->id ? 'selected' : '' ?>>
              <?= $dept->name ?>
            </option>
          <?php endforeach ?>
        </select>
