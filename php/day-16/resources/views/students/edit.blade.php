      ...
      <form action="{{ route('students.update', $student) }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
          ...
          <select id="department" name="department_id"
            ...
            <option value="{{ $department->id }}" {{ old('department_id', $student->department_id)==$department->id ?
              'selected' : '' }}>
              {{ $department->name }}
            </option>
            @endforeach
        ...
        <div x-data="{ fileName: '', preview: '' }" class="w-full">
          ...
          <div
            ...
            <template x-if="!preview && '{{ $student->image }}'">
              <img src="{{ storage_url($student->image) }}" alt="Preview"
                class="mt-2 size-32 rounded object-scale-down shadow" />
            </template>
            <template x-if="!preview">
              <p class="text-sm text-neutral-500 dark:text-neutral-400 py-4">Drag & drop or click to upload</p>
            </template>
            <template x-if="preview">
